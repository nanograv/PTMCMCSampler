...
class AdaptiveMetropolisProposal(MHProposal):
    ...
    def finalize(self, model, backend, iter):
        ...
        if iter !=0 and iter % self.cov_update == 0:
            self._update_covariance(backend, iter)


    def _update_covariance(self, backend, iter):
        """
        ...
        samples = backend.coords[-self.cov_update:,
        ...
        for ii in range(self.cov_update):
            ...
            diff = samples[ii,:] - oldmu
            self.mu += diff / start

            self.M2 += np.outer(diff, samples[ii,:] - self.mu)
        ...
        print '\n'
        print iter, start, self.cov_update
        print np.diag(np.sqrt(self.cov))
        print np.std(backend.coords, axis=0)
        print self.mu
        print np.mean(backend.coords, axis=0)
        print '\n'
    ...
    def get_AM_proposal(self, coords, iteration, beta, pardict=None):
        ...
        q[indexmap] += scale * np.dot(self.U[jumpind],
                                      np.sqrt(self.S[jumpind]) *
